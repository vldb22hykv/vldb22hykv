...
int cceh_numa = 0;
int default_numa_node_offset = cceh_numa;
int numa_node_offset = cceh_numa;

#ifdef CCEH_PERSISTENT
static constexpr char CCEH_PMEM_POOL_FILE[] = "/mnt/pmem/hykv/cceh-allocator.file";
class PMemAllocator {
  public:
...
// Analogously to previous work, we use fingerprinting in order to store keys larger than 8 Byte in the Offset Map
...
constexpr size_t kSegmentSize = (1 << kSegmentBits) * 16 * 4; //64 bytes * 256
...
template <typename KeyType>
struct Segment {
    static const size_t kNumSlot = kSegmentSize / sizeof(Pair); //kSegmentSize = 16384,  sizeof(Pair) = 16
    ...
    void* operator new(size_t size) {
		...
		ret = (void*) numa_alloc_onnode(size, default_numa_node_offset);
        return ret;
#endif
    }

	void* operator new(size_t size, int numa_node) {
#ifdef CCEH_PERSISTENT
        PMEMoid ret;
        PMemAllocator::get().allocate(&ret, size);
        return pmemobj_direct(ret);
        ...
        void* ret;
        //if (posix_memalign(&ret, 64, size) != 0) throw std::runtime_error("bad memalign");
		ret = (void*) numa_alloc_onnode(size, numa_node);
        return ret;
#endif
    }
...
Segment<KeyType>** Segment<KeyType>::Split(void) {
  ...
  split[0] = this;
  split[1] = new (numa_node_offset) Segment<KeyType>(local_depth + 1);
...
CCEH<KeyType>::CCEH(size_t initCap)
    : dir{new (numa_node_offset) Directory<KeyType>(static_cast<size_t>(log2(initCap)))}
{
    for (unsigned i = 0; i < dir->capacity; ++i) {
        dir->_[i] = new (numa_node_offset) Segment<KeyType>(static_cast<size_t>(log2(initCap)));
        dir->_[i]->pattern = i;
    }
}
...
IndexV CCEH<KeyType>::Insert(const KeyType& key, IndexV value, KeyCheckFn key_check_fn) {
    ...
    while (true) {
        ...
        if (ret == 0) {
			// Count the hotness of the segment
			//target->counter = target->counter + 1;
			//if (target->counter % 100 == 0) printf(" %zu", target->counter);
        ...
        { // CRITICAL SECTION - directory update
			...
			//size_t tmp_counter = dir->_[x]->counter;
			//dir->_[x]->counter = tmp_counter + 1;
			//if (tmp_counter % 10 == 0) printf(" %zu",tmp_counter);
            if (dir->_[x]->local_depth - 1 < dir->depth) {  // normal split
                ...
                auto dir_old = dir;
                auto d = dir->_;
                auto _dir = new (numa_node_offset) Directory<KeyType>(dir->depth + 1);
                for (unsigned i = 0; i < dir->capacity; ++i) {
                    if (i == x) {
                        _dir->_[2 * i] = s[0];
                        _dir->_[2 * i + 1] = s[1];
...
IndexV CCEH<KeyType>::Get(const KeyType& key, KeyCheckFn key_check_fn) {
	//Finally enter here, 3rd
    size_t key_hash;
    if constexpr (std::is_same_v<KeyType, std::string>) { key_hash = h(key.data(), key.length()); }
    else { key_hash = h(&key, sizeof(key)); }
    const size_t loc = (key_hash & kMask) * kNumPairPerCacheLine;

	// Check the lock of the current kv bucket
	...
	//printf("kNumPairPerCacheLine * kNumCacheLine %ld\n", kNumPairPerCacheLine * kNumCacheLine);
	//kNumPairPerCacheLine * kNumCacheLine is 16
	//kNumPairPerCacheLine = 4
	// Most time-consuming part here
    ...
    for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
        ...
        if (segment->_[slot].key == key_checker) {
		  ...
		  //if (segment->counter % 100 == 0) printf(" %zu", segment->counter);
		  //segment->counter is 100

          IndexV offset = segment->_[slot].value;
		  //IndexV offset = co_await prefetch(segment->_[slot].value);
