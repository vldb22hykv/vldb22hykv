...
#define DEFINE_VAR_BM(method) \
    BENCHMARK_TEMPLATE2_DEFINE_F(HykvFixture, reclaim_var ##_ ##method, std::string, std::string)(benchmark::State& state) { \
        bm_reclaim(state, *this, #method);  } \
    BENCHMARK_REGISTER_F(HykvFixture, reclaim_var ##_ ##method) GENERAL_ARGS \
...
#define DEFINE_FIXED_BM(method) \
    BENCHMARK_TEMPLATE2_DEFINE_F(HykvFixture, reclaim_fixed ##_ ##method, KeyType16, ValueType200)(benchmark::State& state) { \
        bm_reclaim(state, *this, #method);  } \
    BENCHMARK_REGISTER_F(HykvFixture, reclaim_fixed ##_ ##method) GENERAL_ARGS \
     ->Threads(RECLAIM_NUM_OP_THREADS + 1)->Threads(RECLAIM_NUM_OP_THREADS);
    // Reclaim with extra thread            No reclaim
...
inline void bm_reclaim(benchmark::State& state, VFixture& fixture, std::string method) {
    ...
    for (auto _ : state) {
        ...
        if (should_reclaim && is_reclaim_thread) {
            // For this benchmark, the `reclaim` method needs to be made public in Hykv.
            fixture.getHykv()->reclaim();
