...
void init_bm_data(benchmark::State& state, const size_t storage_type, const size_t bm_type, const size_t data_len) {
    ...
    switch (storage_type) {
        case DRAM_BM: {
            const auto map_flags = hykv::HYKV_DRAM_MAP_FLAGS;
        ...
        case FSDAX_BM: {
            ...
            void* data = mmap(nullptr, data_len, hykv::HYKV_MAP_PROT, hykv::HYKV_MAP_FLAGS, fd, 0);
            check_mmap(data);
        ...
        case DEVDAX_BM: {
            const auto pmem_file = HYKV_POOL_FILE;
