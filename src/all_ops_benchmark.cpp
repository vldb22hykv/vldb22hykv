...
int coru_map_counter = 0;
std::map<long unsigned int, int> thread_id_map;
...
void bm_get(benchmark::State& state, BaseFixture& fixture) {
    ...
    if (is_init_thread(state)) {
		...
		int streams = 16;
		int num_threads = 24;
		throttlers = (throttler*) malloc(num_threads * sizeof (throttler));
		throttler::init_throttler(streams, num_threads);
		for (int i = 0; i < num_threads; i++)
			throttlers[i].throttler_id = i;
#endif
    }

#ifdef COROUTINE
	thread_id_map.insert(std::pair<long unsigned int, int>(pthread_self(), coru_map_counter));
	coru_map_counter++;
#endif
...
#ifdef COROUTINE
    if (is_init_thread(state)) {
		throttler::set_map(thread_id_map);
		/*
		for (std::map<long unsigned int, int>::iterator itr = thread_id_map.begin(); itr != thread_id_map.end(); ++itr) {
				std::cout << '\t' << itr->first
					<< '\t' << itr->second << '\n';
		}
		*/
		//printf("set_map completed\n");
	}
#endif
